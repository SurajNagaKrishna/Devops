...
vi.mock("./pages/EmployeeDashboard", () => ({ default: () => <div data-testid="employee-page">Employee dashboard</div> }));
...
function session(role) {
  api.get.mockResolvedValue({ data: { user: { emp_id: 1, role } } });
}
...
describe("authentication and role navigation", () => {
  ...
  it.each(["/admin", "/manager", "/employee"])("redirects unauthenticated users from %s", async path => {
    setPath(path);
  ...
  it.each([
  ...
  ])("redirects an authenticated %s from the root", async (role, path, page) => {
    session(role);
    ...
    if (role === "Team Manager") {
      await waitFor(() => expect(screen.getByText("Team Manager")).toBeInTheDocument());
    } else {
  ...
  it("returns the user to the root after logout", async () => {
    api.get.mockResolvedValue({ data: { user: { emp_id: 1, role: "Team Manager" } } });
